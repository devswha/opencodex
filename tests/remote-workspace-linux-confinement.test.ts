    ...
    const result = await runner.run({
      ...
      command: [
        ...
        [
          ...
          'try { readFileSync(outsideRead); process.exit(26); } catch {}',
          'try { writeFileSync(outsideWrite, "escaped"); process.exit(27); } catch {}',
          'try { await fetch(`http://127.0.0.1:${port}`, { signal: AbortSignal.timeout(500) }); process.exit(28); } catch {}',
        ].join("\n"),
        "--",
        outsideRead,
        outsideWrite,
